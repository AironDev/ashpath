...
class Escrow extends Model
{
    ...
    {
        return $this->hasMany(Transaction::class, 'transaction_reference_id', 'id');
    }
    ...
    public function createEscrowTransfer($arr)
    {
        $transfer              = new self();
    ...
    public function createEscrowTransaction($arr)
    {
        ...
        $transaction->transaction_type_id      = Escrow;
    ...
    public function updateEscrowWallet($arr)
    {

        if (!empty($arr['transfer_receiver_id']) && isset($arr['userInfo']))
        {
            $receiverWallet = Wallet::whereId( getEscrowWallet() )->first(['id', 'balance']);
    ...
    public function processEscrowConfirmation($arr = [], $clearSessionFrom)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                //Create Transfer * set the status to pending here
                $transfer = self::createEscrowTransfer($arr); //good


                //Create Transferred Transaction * create the transfered record and set status to pending
                ...
                */
                ...
                //Debit Sender Wallet
                self::updateSenderWallet($arr['senderWallet'], $arr['total']); //good


                //Hold on Escrow
                $arr['transfer_receiver_id'] = $transfer->receiver_id;
                self::updateEscrowWallet($arr); //good
                ...
                $transactionOrTransferId =  $transfer->id; // ($clearSessionFrom == 'web') ? $transferredTransactionId : $transfer->id;
