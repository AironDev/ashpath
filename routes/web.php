...
Route::group(['prefix' => Config::get('adminPrefix'), 'namespace' => 'Admin', 'middleware' => ['guest:admin', 'locale', 'ip_middleware']], function ()
{
    ...
    //Admin Can deposit for a userll
    ...
    //Currency PaymentMethod Settingsj
    ...
    // Escrow  Settings
    Route::match(['get', 'post'], 'settings/escrow', 'SettingController@escrow');


     // Admin Security Settings
    Route::match(['get', 'post'], 'settings/admin-security-settings', 'SettingController@adminSecuritySettings');
    ...
    Route::group(['middleware' => ['check-enabled-currencies-preference']], function ()
    {
        ...
        Route::get('settings/coin-credentials/blockio/{network}', 'CryptoCurrenciesSettingController@coinCredentials');
    });
});
...
Route::group(['middleware' => ['guest:users', 'locale', 'twoFa', 'check-user-inactive'], 'namespace' => 'Users'], function ()
{
    ...
    Route::group(['middleware' => ['permission:manage_deposit', 'check-user-suspended']], function ()
    {
        ...
        Route::post('deposit/paystack-confirm-payment', 'DepositController@paystackConfirm')->name('deposit.paystack.confirm');
        
        Route::get('deposit/paystack-payment/success', 'DepositController@stripePaymentSuccess')->name('deposit.paystack.success');
    ...
    //ESCROW - With Suspend Middleware
    Route::group(['middleware' => ['permission:manage_transfer', 'check-user-suspended']], function ()
    {
        Route::match('GET', 'create-escrow', 'EscrowController@create');
        
        Route::post('store-escrow', 'EscrowController@create');
        // Route::post('transfer-user-email-phone-receiver-status-validate', 'MoneyTransferController@transferUserEmailPhoneReceiverStatusValidate');
        // Route::post('amount-limit', 'MoneyTransferController@amountLimitCheck');
        
        Route::get('store-escrow-confirm', 'EscrowController@sendMoneyConfirm');
    });
    ...
    {
        Route::get('wallet-list', 'CryptoSendController@crypto');
